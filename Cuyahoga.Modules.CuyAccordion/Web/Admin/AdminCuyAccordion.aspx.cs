	...
	//Create an instance of this module...
        ...
        private void DatabindAccordionItems(int accordionId)
        {
            if (this._Module._cuyAccordionDao.AccordionExists(accordionId))
            {
                ...
                gvAccordionItems.DataSource = this._Module._cuyAccordionDao.GetActiveAccordionSpecificItems(a);
        ...
        #region gvAccordionItems
            ...
            protected void gvAccordionItems_RowDeleting(object sender, GridViewDeleteEventArgs e)
            {
                ...
                foreach (AccordionItem item in a.Accordion.AccordionItems)
                {
                    if (item.ZIndex > a.ZIndex)
                    {
                        item.ZIndex--;
                    }
                }
                a.Accordion.AccordionItems.Remove(a);
                this._Module._cuyAccordionDao.SaveOrUpdateAccordion(a.Accordion);
                DatabindAccordionItems(accordionId);
            }

            protected void gvAccordionItems_RowCommand(object sender, GridViewCommandEventArgs e)
            {
                int itemId;
                AccordionItem ai;
                AccordionItem aid;
                ...
                switch (e.CommandName) { 
                    case "MoveUp":
                        ...
                        ai = this._Module._cuyAccordionDao.GetAccordionItem(itemId);
                        if (ai.ZIndex > 0) 
                        {
                            aid = ai.Accordion.AccordionItems[ai.ZIndex - 1] as AccordionItem;
                            aid.ZIndex += 1;
                            ai.ZIndex += -1;

                            this._Module._cuyAccordionDao.SaveOrUpdateAccordionItem(ai);
                            this._Module._cuyAccordionDao.SaveOrUpdateAccordionItem(aid);
                    ...
                    case "MoveDown":
                        ...
                        ai = this._Module._cuyAccordionDao.GetAccordionItem(itemId);
                        if (ai.ZIndex < (ai.Accordion.AccordionItems.Count - 1))
                        {
                            aid = ai.Accordion.AccordionItems[ai.ZIndex + 1] as AccordionItem;
                            ai.ZIndex += 1;
                            aid.ZIndex += -1;
